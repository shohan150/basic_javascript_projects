...
createBtn.addEventListener('click', () => {
    let inputBox = document.createElement('div');  //used let in tutorial
...
notesContainer.addEventListener('click', function (e) {
    ...
    // sets up an event listener for a click event on an element with the id "notesContainer". When a click event occurs within this container, the code checks if the target of the click event is an IMG (image) element. If it is, it removes the parent element of the image, effectively removing the image.

    // .addEventListener('click', function (e) { ... }): This code adds a click event listene. The e parameter represents the event object. if (e.target.tagName === 'IMG') { ... }: Inside the event listener, it checks if the target of the click event (e.target) is an IMG element. The tagName property of the target element returns its tag name in uppercase.
...
document.addEventListener('keydown', event => {
    ...
    // When a key is pressed down, the event handler checks if the pressed key is the "ENTER" key (usually the "Enter" or "Return" key on the keyboard). If the "ENTER" key is detected, the code executes two actions:
